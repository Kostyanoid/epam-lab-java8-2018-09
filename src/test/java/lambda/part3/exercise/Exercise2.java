...
class Exercise2 {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> map(Function<T, R> mapping) {
            List<R> changedSource = new ArrayList<>();
            this.getMapped().forEach(e -> changedSource.add(mapping.apply(e)));
            return MapHelper.from(changedSource);
        }
        ...
        public <R> MapHelper<R> flatMap(Function<T, List<R>> flatMapping) {
            List<R> changedSource = new ArrayList<>();
            this.getMapped().forEach(e -> changedSource.addAll(flatMapping.apply(e)));
            return MapHelper.from(changedSource);
        }
    }
